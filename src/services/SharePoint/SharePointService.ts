...
export class SharePointServiceManager {
    ...
    public getListItem(listId: string, itemId: number){
        return this.get(`/_api/lists/getbytitle('${listId}')/items(${itemId})?$select=*,Author/Name,Author/Title,Author/EMail,LinkToIdea/Title&$expand=Author/Id,LinkToIdea/Id,AttachmentFiles`);
    }
    ...
    public getListItemVersions(listId: string, itemId: number){
        ...
        return this.get(`/_api/lists/getbytitle('${listId}')/items(${itemId})/versions?$select=*,Author/Name,Author/Title,Author/EMail,LinkToIdea/Title&$expand=Author/Id,LinkToIdea/Id,AttachmentFiles&$orderby=Created asc`);
    ...
    public updateElemSpec(name, desc, formula, status){
        ...
        return this.context.spHttpClient.fetch(`${this.context.pageContext.web.absoluteUrl}/_api/lists/getbytitle('${this.elSpeclistID}')/items(${this.elSpecItemID})`, SPHttpClient.configurations.v1,
        {
            headers: {
                'Accept': 'application/json;odata=nometadata',
                'Content-type': 'application/json;odata=verbose',
